...
| 17 | [Semiconductor supply-chain layers](17-semiconductor-layers/) | Which layer leads, how does money flow across them, and does price drive the chatter? | Across 83 names (≥$100M): leadership sits on the periphery (power +1,902% vs hyperscalers +175%); layers co-move (no chain lead-lag, corr +0.80 @ lag 0); price drives the chatter (posts chase +3–22% run-ups) which then rides momentum (+2.4% fwd alpha; curated X +7.3%). |
