...
![positioning](../figures/co_CHAI.png)
...
Its edge (real, narrow): (i) a fast model ladder - Chai-2 hit 16% full-length de-novo antibody success, and Chai-3 is claimed to roughly double that (matching approved-drug-grade binding in ~50% of cases) with ~100x tighter binding [reported/disclosed, internal, no external benchmark]; the product-strength claim is that Chai-2 compressed months of wet-lab antibody work into ~two weeks [reported]; (ii) pure-picks positioning as trust signal - no competing pipeline, which is what won Eli Lilly (Jan 2026) and Pfizer (Jun 2026) partnerships plus "15+" pharma in talks [reported]; (iii) frontier-lab pedigree + OpenAI halo that pulls capital and talent. The edge is relationship + go-to-market posture, not model supremacy - a critical distinction for the moat.
...
## Core variables
1. **Partnership economics realized, not signed.** Do Lilly/Pfizer deals convert past 1-3% upfronts into milestone/royalty cash - i.e. does a Chai-designed molecule *advance*? Logos are free; paid progression is the thesis. Currently unobservable (private, no terms). The realization clock is long by the partners' own framing: Lilly's CIO put "mid-2030s, if not late-2030s" on when an AI-developed medicine actually reaches market [reported, Mar 2026] - so milestone/royalty proof is years out, and near-term the mark rests entirely on deal count and model claims, not cash.
...
Coverage on a pre-revenue private stays thin; these are the concrete, dated points that move the read. Nothing below changes the grade - it hardens it.

- **Series B closed, $130m at $1.3bn post (14-15 Dec 2025)** [disclosed]. Co-led by General Catalyst and Oak HC/FT; OpenAI, Menlo, Dimension, Thrive and others in. Total raised >$225m. (Already in the table above; anchor for the markup math.)
- **Eli Lilly partnership (Jan 2026)** [reported]. Access to Chai's platform for biologic drug development; no financial terms disclosed. Lilly separately funds Isomorphic - the same partner is building the in-house AF3-class capability that can envelop the supplier at renewal.
- **Pfizer partnership (announced 4 Jun 2026)** [reported]. Pfizer gets deployment rights to Chai's generative platform, a custom model trained on Pfizer's proprietary data, and early access to Chai-3 - described as "among the first" to reach Chai-3. Deployment rights, not equity or exclusivity; no financial terms disclosed. This is the concrete form the earlier "Pfizer (2026)" placeholder now takes: a data-attach/workflow deal, which is the switching-cost vector to watch, but with undisclosed economics it remains a logo until a milestone triggers.
- **Chai-3 claims (mid-2026)** [reported/disclosed, internal - no external benchmark]. Roughly double Chai-2's success rate, hitting approved-drug-grade binding in ~50% of cases (vs 16% for Chai-2), with binding ~100x tighter. Detailed benchmarking not publicly disclosed, so the number is company-sourced; the ~12-month open-clone half-life still applies to whatever edge this represents.
- **~$3.4bn round still in talks, NOT closed (as of Jun 2026)** [reported]. ~$400m sought, no lead selected. Treat the $3.4bn mark as an aspiration, not a print - a ~2.6x step-up in ~6 months on zero revenue that has not cleared.
- **Timeline framing** [reported, Mar 2026]. Lilly's CIO put an AI-developed medicine reaching market at "mid-2030s, if not late-2030s" - i.e. the milestone/royalty cash that would prove the thesis is years away; the interim is priced on logos and claims.

Net: the direction of travel is more logos (Pfizer added), stronger model claims (Chai-3), and a higher aspirational mark - but zero disclosed economics, no closed up-round, no external Chai-3 benchmark, and a partner-stated payoff horizon in the 2030s. The evidence added since the last cut reinforces "impressive demo, feature not franchise," it does not refute it.
