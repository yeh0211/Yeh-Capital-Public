...
Sits at L5a interpretive triage (the model that reads the scan and raises the flag) and L6 clinical workflow (routing, notification, worklist prioritization inside the PACS/EHR loop). What flows in: DICOM images off the modality (L1/L2) plus the PACS feed; what flows out: a prioritized worklist, a mobile/desktop alert to the on-call clinician, and increasingly a draft report - the draft-reporting push is now a named product, "First Read," which drafts preliminary radiology report text from chest radiographs and won FDA Breakthrough Device Designation on 25 Jun 2026 [disclosed]. Note the tier: a Breakthrough designation is an expedited-review lane, not a clearance, and First Read keeps clinician review and final sign-off on every report - so the move deeper into the radiologist's output is real in intent but not yet a cleared, autonomous product. Named-patient liability stays with the reading physician (assistive tier) - Aidoc carries little of the med-mal weight that would justify a defensive moat on its own. The aiOS move is an attempt to become the L6 orchestration rail other vendors rent, i.e. to stop being a single L5a app and become the layer above them.

Crowded L5a triage: Viz.ai (the direct rival, stroke/PE care-coordination, similar system-selling motion), RapidAI, Cleerly (adjacent cardiac CT), plus the OEM stacks that own the modality and the PACS - GE HealthCare (~120 AI clearances), Siemens (~89), Philips (~50), Canon (~45) - any of which can bundle triage into the scanner/PACS it already sells. Aidoc's edge is breadth + installed base + the platform pivot: most FDA clearances rolled into one multi-condition triage workflow (the Jan-2026 "first comprehensive foundation-model triage" clearance, 11 new + 3 prior indications [disclosed], covering 14 acute conditions on abdomen CT for the ED/ambulatory setting), the widest deployment footprint, and aiOS as a multi-vendor governance layer that raises switching cost beyond any single algorithm. The multi-vendor claim now has named evidence rather than being aspirational: aiOS hosts third-party imaging-AI vendors alongside Aidoc's own models - Cercare Medical (stroke perfusion) and Circle CVI (cardiac) were added in Dec-2025 [reported] - which is the observable form of the "governance rail others rent" thesis (the test is whether hosted-vendor count and stickiness compound, per CORE-1). The hyperscaler tie-ins (NVIDIA compute + capital, AWS compute + capital) are real distribution/scale access most single-point rivals lack.
...
The 2025-2026 stretch moves the story along both axes the thesis watches - the CARE model / draft-reporting up-stack, and the aiOS multi-vendor rail - without yet settling the one number that decides the moat (NRR-at-price, still unobservable while private).

- **29 Apr 2026 - Series E, $150m [disclosed].** Led by Growth Equity at a sell-side Alternatives arm; General Catalyst, SoftBank Vision Fund 2, and NVIDIA's venture arm participated. Brought cumulative funding above $500m ("~$520m" cited). Reporting put ARR at "multi-tens of millions" and the CEO declined the exact figure [reported] - consistent with the ~$60m (2025) already carried here. Valuation was again not disclosed; a single secondary source floated a >$1bn mark [unverified], which the company and the lead reporting did not confirm - so the undisclosed-mark caution flag stands. Capital use named: further CARE build, more clinical indications, automated draft reporting, and aiOS global rollout. IPO is under consideration post-round [reported].
- **Jan 2026 - CARE triage clearance [disclosed].** FDA cleared the first comprehensive foundation-model triage - 11 new + 3 prior indications, 14 acute conditions on abdomen CT for the ED / ambulatory setting - which the company frames as an order-of-magnitude cut in false alerts vs single-condition tools [reported]. This is the clearance that upgrades the app from a stack of point algorithms to one multi-condition workflow; per the Moat-1 audit it still gates the shelf, not the revenue.
- **25 Jun 2026 - "First Read" Breakthrough Device Designation [disclosed].** AI that drafts preliminary radiology report text from chest radiographs, built on the same architecture as the cleared abdominal CT triage app. Important tier distinction: a Breakthrough designation expedites review, it is not a clearance - and the product keeps clinician review and final approval on every report. This is the concrete first step of the "touch the radiologist's output / productivity" monetization path in CORE-2, but it is a designation, not yet a shipped, cleared, billable capability.
- **2025-2026 - aiOS multi-vendor evidence and systemwide deployments [reported].** Third-party imaging-AI vendors were added to aiOS (Cercare Medical, Circle CVI in Dec-2025), the first named data points for the "governance rail others rent" claim behind CORE-1. On the footprint side, health-system-wide aiOS rollouts were reported at Sutter Health and WellSpan (9 hospitals, 250+ care locations, 21 new AI care pathways activated), extending breadth but, again, not disclosing renewal-cycle pricing.

Net read on the updates: they strengthen the two things the profile already credits - breadth and the early shape of an owned rail - and add the first tangible proof that aiOS hosts outside vendors (CORE-1) and that the up-stack into reporting has begun (CORE-2, via First Read). None of it yet supplies the load-bearing proof - audited net-revenue-retention at price through a renewal cycle - so the verdict below is unchanged.
