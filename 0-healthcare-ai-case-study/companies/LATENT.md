...
![positioning](../figures/co_LATENT.png)
...
Latent's edge is two-fold and real-but-narrow: (i) **founder pedigree** (AlphaFold2 core team) as a talent/recruiting and credibility asset; (ii) **a genuine lab-validated benchmark** - the July 2025 model (now retro-named **Latent-X1**) reports **10-64% mini-binder hit rates to picomolar affinity** and **91-100% macrocycle hit rates to single-digit-micromolar**, across **7 therapeutic targets**, generation **>10x faster / within seconds**, claimed state-of-the-art vs prior generative tools [disclosed]. The distribution edge (no-code, in-browser) is a real go-to-market wedge - it lowers adoption friction for biotechs without ML teams, exactly the buyer Kohl names.

Where the competition is closing: the L9b design frontier is now a multi-cohort race, not a moat. Antibody-specific generative design (Latent's late-2025 push, see below) overlaps directly with Generate:Biomedicines, Absci, Nabla Bio, Chai Discovery and the ESM3/EvolutionaryScale line, while the open-weights wave (Boltz-1/2, Chai-1, Protenix, OpenFold-3) keeps resetting the free-tier anchor. What Latent has added since the profile was first written - an autonomous text-prompt agent and an infra partnership (below) - is engineering leverage, not a new moat layer; every peer can license the same GPUs.
...
Real product cadence since the profile was written - three releases in eight months - but the two thesis gaps (no priced/public revenue, no proven compounding data moat) are still open.

- **16 Dec 2025 - Latent-X2 (antibodies).** The company moved up the value chain from mini-binders/macrocycles into antibodies. Reported [disclosed]: confirmed hits on **9 of 18 targets (50%)** selected for difficulty, **picomolar-to-nanomolar** affinities, only **4-24 designs per target**, and - the notable claim - the **first AI-generated antibody to clear a 10-donor human ex-vivo T-cell immunogenicity panel**, with developability "comparable to approved therapeutics" without optimisation. Access: web platform + API, "available to selected partners" (partnerships email), **no public pricing** [reported]. Read: this is a deliberate step toward the L9c *selection* layer (developability/immunogenicity de-risking, not just raw binding) the profile said Latent had chosen not to play - a partial hedge of the pure-L9b positioning, but still self-reported and still un-priced.
- **March 2026 - Latent-Y (autonomous agent).** An AI agent that turns a natural-language therapeutic goal into lab-ready candidate sequences (target analysis -> epitope selection -> design -> computational validation), powered by Latent-X2. Reported [disclosed]: **67% target-level success across 9 targets**, confirmed nanobody binders on **6 targets** at single-digit-nanomolar, and one campaign that produced confirmed transferrin-receptor binders from a single published paper with "no manual curation." A workflow-automation layer on the same model - lowers the buyer's skill bar further, but does not by itself create the owned, non-replicable input the moat needs.
- **March 2026 - NVIDIA + Nebius infrastructure partnership.** Training/inference moved onto **NVIDIA Blackwell B300** GPUs via Nebius, with **>4x training speedup and >60% faster inference** vs prior-generation hardware [reported]. Compute leverage, not competitive moat - the same silicon is rentable by every L9b peer.
- **Funding / commercials - unchanged.** Total raised still **$50m** ($40m Series A, Feb 2025); **no new round disclosed**, no priced API, **no named customer/partner signings**, no disclosed revenue as of this update [reported/unverified]. Valuation still undisclosed.

Net: the product is materially better and now spans antibodies plus an agent wrapper, but on the two variables that decide the thesis - **priced, sticky monetisation** and a **compounding proprietary data asset** - nothing has yet moved from intent to proof. Coverage remains partly thin (private, self-reported benchmarks, no independent replication, no financials).
