...
![positioning](../figures/co_RETRO.png)
...
Sits at L9a (target/biology selection - which aging mechanism to attack: autophagy, reprogramming, plasma) and L9c (candidate selection) as a vertically-integrated developer sitting ON a modality (reprogramming), not selling the modality. What flows IN: the reprogramming toolkit + AI-designed proteins (the "model/enzyme," an increasingly commodity input - RetroSOX/RetroKLF came from OpenAI's GPT-4b micro, not Retro-proprietary IP). What flows OUT (intended): owned clinical assets - RTR242 (oral autophagy restart, Phase I, ~8 participants dosed Dec 2025, first-in-human in ~15 months from indication selection [disclosed]); microglia cell therapy (human trials planned 2026); HSC reprogramming (2027); tissue reprogramming (pre-clinical). Value forms one layer down from the tool, in the owned asset - the same "the model is the door, not the moat" structure as Isomorphic and the gene-editing cohort.

Competitively, a crowded, extremely well-capitalized field: Altos Labs (~$3bn+ raised), Calico (Alphabet, ~$3.5bn+ - but its lead ALS drug FAILED Jan 2025), NewLimit (~$247m, also reprogramming), BioAge (public, halted its obesity program Dec 2024) [reported]. Against those balance sheets Retro is a minnow (~$180m + an undisclosed 2026 close). Its edge is (i) speed - clinical-stage in ~3 years, cheap-and-fast lab builds; (ii) the Altman/OpenAI axis - privileged AI access (GPT-4b micro delivered a >50-fold jump in reprogramming-marker expression, iPSC generation compressed ~3 weeks -> ~7 days [disclosed]); (iii) breadth - three independent shots (autophagy, reprogramming, plasma). The AI result is a genuine efficiency win at the L9b generation step - but that is the cheap, already-solvable half of the funnel.
