...
![positioning](../figures/co_ISOMORPHIC.png)
...
The frontier-lab cohort splits by disclosed model: **drug-owners** (Isomorphic - the hybrid - Xaira, Generate:Biomedicines, Genesis, Recursion, Retro) vs **tool vendors** (Chai Discovery, EvolutionaryScale/ESM3, NVIDIA BioNeMo, Latent Labs). Crowding is at the antibody/binder-**generation** frontier and the model-vendor layer - the step that carries no NPV - not at selection/clinical translation, where value sits. Its edge is the only techbio pairing Alphabet/AlphaFold provenance with three top-pharma partners (Lilly, Novartis, J&J) and no pharma equity on the cap table - a genuine access/optionality asset most peers lack. The three partners buy different things: **Lilly** (up to $1.7B) and **Novartis** (up to $1.2B) are small-molecule target-discovery deals; the **J&J** tie is a **cross-modality, multi-target** collaboration with **terms undisclosed** [reported]. Operationally the lab now runs across **three sites** - **London HQ, Cambridge MA, and Lausanne CH** [reported] - a footprint that reads as a build toward owned clinical development, not a compute-only shop. The one honest technical claim is IsoDDE's out-of-distribution generalization (50% vs AlphaFold3's 23.3% on <20%-similarity protein-ligand cases), from a **technical report published Feb 2026** - **[estimate - vendor-reported, un-replicated];** treat as vendor-reported until a third party benchmarks it. One external academic (Mohammed AlQuraishi) called the report "on the scale of an AlphaFold 4" [reported - third-party opinion, not a benchmark].
...
2. **[CORE] Own-pipeline progression** - **oncology** is the first internal program into the clinic [reported]; the near-term gate is an **IND filing targeted by end-2026**, then first-in-human dosing, with first Phase II efficacy readout ~2028-29. As of the **May 2026** Series B, **no patient had been dosed** with an Isomorphic-designed compound [reported]. The end-2026 target already slipped a year from end-2025. Zero human efficacy data exists; the entire valuation is forward.
...
Chronology of the concrete, dated moves. None of these is human efficacy; the master variable (Phase II PoS) is still unobservable before ~2028-29.

- **31 Mar 2025** - first external round: **$600M**, Thrive-led, Alphabet/GV participating [disclosed]. Stated use: scale the drug-design engine and advance internal candidates.
- **Feb 2025** - **Novartis** partnership expansion announced [reported]; the J&J tie is later framed as a **cross-modality, multi-target** collaboration, **terms undisclosed** [reported].
- **Jul 2025** - President Colin Murdoch says human trials are "in sight" and the company is "staffing up" for oncology; still no dosing [reported].
- **Feb 2026** - **IsoDDE technical report published** [disclosed]: the vendor OOD figure (50% vs AlphaFold3's 23.3% on <20%-similarity protein-ligand cases) traces to this report. One external academic characterized it as "on the scale of an AlphaFold 4" [reported - opinion, not an independent benchmark]. Still **un-replicated by any third party**.
- **12 May 2026** - **$2.1B Series B** closes, Thrive-led; new backers MGX, Temasek, CapitalG, UK Sovereign AI Fund alongside Alphabet/GV [disclosed]. Total external capital to **~$2.7B** across ~14 months, **no pharma equity** on the cap table. Stated use: scale IsoDDE and push first internal candidates toward the clinic.
- **As of ~31 May 2026** - reported headcount **~379** [reported] (vs the profile's ~350-400 [estimate] band); hiring across **London, Cambridge MA, Lausanne CH**.
- **Near-term gate (targeted end-2026)** - first **IND filing**, then first-in-human dosing in **oncology**. As of May 2026, **no patient dosed** [reported]. This is the first observable, binary company event; everything material after it (Phase II efficacy) is ~2028-29.

Coverage note: secondary press repeats a "17 internal programs incl. cardiovascular" line; primaries still say oncology + immunology with no disclosed count, so that figure stays **[unverified]**. No traded stub exists; nothing here changes the exposure path (private rounds or Alphabet).
