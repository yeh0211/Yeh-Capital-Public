...
| `media_catalog.csv` / `.json` | 186 | The show through independent eyes — Computex 2024–2026 booth walkthroughs and hands-on coverage from the tech press (KitGuru, der8auer, PCWorld, Paul's Hardware, Gamers Nexus, Hardware Unboxed, ShortCircuit…) and vendor channels, tagged `company` vs `third-party`. Links-only index; coverage is best-effort (top reviewer videos), not exhaustive. |
